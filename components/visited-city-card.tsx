...
export const VisitedCityCard = memo(function VisitedCityCard({
  visit,
  showCountry = true,
  actionLabel = "OPEN",
  onAction,
  ...
  const logged = new Set(
    visit.places.map(
      (place) => `${place.type}:${place.name.toLocaleLowerCase()}`,
    ),
  );
      ...
      <View style={styles.counts}>
        <View style={styles.countItem}>
          ...
          <Text style={styles.countText}>
            {sights.length} {sights.length === 1 ? "sight" : "sights"}
          </Text>
        </View>
        <View style={styles.countItem}>
          <Ionicons
        ...
        <View style={styles.details}>
          ...
          {visit.places.map((place) => (
            <View key={place.id} style={styles.place}>
              <Ionicons
                name={place.type === "airport" ? "airplane" : "camera"}
                size={16}
                color={BrandColors.copperDark}
              />
            ...
            <>
              ...
              <View style={styles.chips}>
                {suggestions.map((suggestion) => {
                  const selected = logged.has(
                    `${suggestion.type}:${suggestion.name.toLocaleLowerCase()}`,
                  );
                  return (
                    <Pressable
                    ...
                    >
                      <Ionicons
                        ...
                        size={14}
                        color={selected ? BrandColors.white : BrandColors.ink}
                      />
                      <Text
                        style={[
                          styles.chipText,
