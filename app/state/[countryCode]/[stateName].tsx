...
export default function StateScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(
        reason instanceof Error ? reason.message : "Could not load this state.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const stateVisits = visits.filter(
    (visit) =>
      ...
      visit.subcountry.trim().toLocaleLowerCase() ===
  ...
  const cityItems: PlaceListItem[] = visitedCities.map((city) => {
    const catalogCity = detail?.cities.find(
      (item) => String(item.id) === String(city.id),
    );
    const matchingVisits = stateVisits.filter(
      (visit) =>
        visit.cityName.trim().toLocaleLowerCase() ===
      ...
      >
        ...
        {detail ? (
          <>
                ...
                items={[
                  {
                    ...
                    value: Math.max(detail.stats.cities, visitedCities.length),
                    label: "CITIES",
                  },
                  {
                    icon: "camera-outline",
                    value: Math.max(
                      detail.stats.sights,
                      completedSightIds.filter((id) =>
                        sights.some((sight) => sight.id === id),
                      ).length,
                    ),
                    label: "SIGHTS",
                  },
                  {
                    ...
                    value: Math.max(detail.stats.airports, localAirportCount),
                    label: "AIRPORTS",
                  },
                ]}
              />
            ...
            <CitiesVisitedSection
              items={cityItems}
              emptyText={`Your visited cities in ${detail.name} will appear here.`}
              onOpen={(city) =>
                router.push({
                  pathname: "/city/[id]",
                  params: {
